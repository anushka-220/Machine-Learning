...
TODO: 
1. display an image in the window and update it.
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        to_canny.pack()
        to_kmeans = tk.Button(self, text="K-Means",
                            command=lambda: controller.show_frame("KMs"))
        to_kmeans.pack()
...
class KMs(tk.Frame):
    def __init__(self, parent, controller):
        ...
        denoise_button = tk.Button(self, text="denoise",
    ...
    def denoise(self):
        im_array = cv.fastNlMeansDenoisingColored(np.array(self.image), None,5,5,3,10)
        ...
        mini = [i for i in clt.cluster_centers_ if (i.sum() == np.min([i.sum() for i in clt.cluster_centers_]))][0]
        maxi = [i for i in clt.cluster_centers_ if (i.sum() == np.max([i.sum() for i in clt.cluster_centers_]))][0]
        ...
        for y in range(0,h):
            for x in range(0, w):
                ...
                b = pix_arr[2]
                delta = 12
                if (((mini[0] - delta) <= r <= (mini[0] + delta)) or ((mini[1] - delta) <= g <= (mini[1] + delta)) or ((mini[2] - delta) <= b <= (mini[2] + delta))):
                    pix_val[x,y] = [222,131,45]
                if (((maxi[0] - delta) <= r <= (maxi[0] + delta)) or ((maxi[1] - delta) <= g <= (maxi[1] + delta)) or ((maxi[2] - delta) <= b <= (maxi[2] + delta))):
